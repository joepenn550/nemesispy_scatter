...
#         'Lowest layer base altitude not contained in atmospheric model'
    ...
    if layer_type == 1: # split by equal log pressure intervals
        ...
        P_base = np.exp(np.linspace(np.log(bottom_pressure),np.log(P_model[-1]),\
            NLAYER+1))[:-1]
...
def average(planet_radius, H_model, P_model, T_model, VMR_model,A_model,PARA_model, ID, H_base,
    ...
    for ilayer in range(NLAYER):
        ...
        K = 1.37947E-23 #??
        dU_dS_int = P_int/(K*T_int) # ideal gas law P = rho*k*T
...
#         VMR_int = np.zeros((NSIMPS, Ngas))
        ...
        for J in range(Ngas):
            ...
            VMR_layer[ilayer, J] \
                = simps(P_int*VMR_int*dU_dS_int,S_int)/U_layer[ilayer]/P_layer[ilayer]
        for J in range(Nmodes):
            A_int[:,J] = np.interp(H_int,H_model,A_model[:,J])
            A_layer[ilayer,J] = simps(A_int[:,J]*dU_dS_int,S_int)#/U_layer[ilayer]
    
    # Scale back to vertical layers
    for ilayer in range(NLAYER):
        U_layer[ilayer] = U_layer[ilayer]/scale[ilayer]
        VMR_layer[ilayer] = VMR_layer[ilayer]
#     print(A_layer)
...
def calc_layer(planet_radius, H_model, P_model, T_model, VMR_model,A_model,PARA_model, ID, NLAYER,
    ...
    H_layer,P_layer,T_layer,VMR_layer,U_layer,A_layer,PARA_layer,dH,scale \
        = average(planet_radius, H_model, P_model, T_model,
            VMR_model,A_model,PARA_model, ID, H_base, path_angle=path_angle,
            H_0=H_0)
#     print(H_layer,P_layer,T_layer,VMR_layer,U_layer,A_layer,dH,scale)
    return H_layer,P_layer,T_layer,VMR_layer,U_layer,A_layer,PARA_layer,dH,scale