...
class NEMESIS:
    ...
    def read_nemesispy_input(self,filepath):
        with open(filepath, "r") as f:
            ...
            if self.latitude_clen > 0:
                self.latitude_smoothing = True
            else:
                self.latitude_smoothing = False
            try:  
                ...
                if self.use_numba_cache:
                    os.environ['USE_NUMBA_CACHE'] = 'True'
                else:
                    os.environ['USE_NUMBA_CACHE'] = 'False'
            except:
                self.output_style = 0
                os.environ['USE_NUMBA_CACHE'] = 'True'
            
            # Now that we have decided whether to use the cache, import what we need
    ...
    def run_nested_sampling(self,frac_remain = 0.01,maxdiff = 5, downsampling = 1):
        self.load_nemesis() 
        self.nested_sampling(frac_remain = frac_remain,maxdiff = maxdiff, downsampling = downsampling)
        
    def load_nemesis(self):
        """
        This scans through the provided folder and reads in all the necessary NEMESIS input files.
        It assumes that folders which are to be smoothed together use the same wavelength grid, aprioris, etc.        
        
        All the threads read all the input files - this turned out to be more stable
        than reading with the master thread and broadcasting, whilst being a similar speed.
        """
        if self.folder_flag:
        ...
        for j in range(self.size+1):
            ...
            for i, subfolder in enumerate(sorted_directories):

                subfolder = self.directory+subfolder+'/'
                try:
                    spx_data, FWHM, latitude, longitude, sol_angs, emiss_angs, aphis = parse_spx_file(subfolder+project+'.spx')
                    if self.rank == 0:
                ...
                if count == 0:
                    ...
                    try:
                        PARA_model = parse_other_ref_file(subfolder+'parah2.ref',buffer=1)
                        PARA_model = PARA_model[:,0]
                    except Exception as e:
                    ...
                    try:    
                        ispace, iscat, ilbl, woff, ename, niter, \
                        philimit, nspec, ioff, lin = parse_inp_file(subfolder+project+'.inp')
                    except Exception as e:
                        if self.rank == 0:
                            print(e,flush=True)
                    ...
                    try:
                        mu, wtmu, nf, nphi, insol, soldist, lowbc, \
                        galb, tsurf, H0, nlay, laytype, layint = parse_set_file(subfolder+project+'.set')
                    except Exception as e:
                        if self.rank == 0:
                            print(e,flush=True)
                    ...
                    custom_p_base = None    
                    if laytype == 4:
                        custom_p_base = parse_lay_file(subfolder+'pressure.lay')
                    
                    angles = np.array(emiss_angs)
                    total_ngeom += len(angles)
                    spectrum = np.stack(spx_data).swapaxes(1,2)
                    spectrum[:,0]+=woff
                    spectra.append(spectrum)# ASSUMES SPECTRA ARE ON SAME WAVE GRID
                    single_wave_grid = spectrum[0,0]

#                     try:
                    xa, xa_err, get_profile, contflags, varflags, haze_wave_grids\
                    ,size_flags, n_real_flags = parse_apr_file(subfolder+project+'.apr',npro,vpdict)
#                     except:
#                         print('Could not read .apr file. Aborting.',flush=True)
#                         self.comm.Abort(1)
        ...
        xa = np.concatenate([xa for i in range(self.nspec)])
        self.xa = np.log(xa)
        xa_err = np.concatenate([xa_err for i in range(self.nspec)])
...
#         if self.rank == 0:
#             if self.niter>0:
#                 np.save('kkstest.npy',kks1.toarray())#*self.dx[None,:]+yn1[:,None])
#             np.save('xa.npy',xn1)
...
#                 np.save('yn.npy',yn1)
    ...
    def nested_sampling(self,frac_remain=0.01,maxdiff = 5, downsampling = 1):
        ...
        self.varlims = np.array([[self.xa[i] - maxdiff*self.xa_err[i],
                                  self.xa[i] + maxdiff*self.xa_err[i]] for i in self.vars_to_vary])
        
        sampler = ultranest.ReactiveNestedSampler([str(i) for i in self.vars_to_vary], self.LogLikelihood, self.Prior,
                  log_dir=self.mre_out+'/ultranest', resume='overwrite')
        self.result = sampler.run(frac_remain=0.01,max_num_improvement_loops=-1)
