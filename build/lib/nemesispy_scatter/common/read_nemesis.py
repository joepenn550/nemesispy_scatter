...
def parse_ref_file(filepath):
    ...
    for line in lines:
        ...
        # Check for the header line and start parsing after it
#         if "height" in line and "press" in line and "temp" in line:
#             start_parsing = True
#             continue
...
def parse_hazef_file(file_path):
    with open(file_path, "r") as haze_f:
        ...
        calc_waves = []
        for i in range(2):
            xai, xa_erri = haze_f.readline().split() # need to sort out 3-parameter distributions?
        ...
        for i in range(int(nwave)):
            v, xai, xa_erri = haze_f.readline().split()
            if not stop:
                params.append(float(xai))
                param_errs.append(float(xa_erri))
    ...
    return params, param_errs, haze_waves, calc_waves, float(clen), float(vref), float(nreal_ref), float(v_od_norm)
