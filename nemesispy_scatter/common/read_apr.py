...
def parse_apr_file(file_path, npro, vpdict):
    """
    This reads in a .apr file and returns the apriori state vector with errors,
            ...
            if len(line) == 3:
                ...
                if param_id == 0:
                    ...
                    with open(folder+cont_filename, "r") as cont_f:
                        ...
                        for _ in range(npro):
                            line = cont_f.readline().split()
                            _, xai, xa_erri = line[:3]
                            xa.append(float(xai))
                    ...
                    if iso_id == -1:
                        size_flags.append(var_id) 
                    if iso_id == -2:
                        n_real_flags.append(var_id)
                elif var_id == 444:
                    # Processing 444 file
                    haze_filename = f.readline().replace('\n', '').strip()
                    haze_params, haze_param_errs, haze_dists, haze_dist_mults, haze_waves, calc_waves, clen,\
                    ...
                    for i in range(len(haze_params)):
                        ...
                        try:
                            dists.append(haze_dists[i])
                            dist_mults.append(haze_dist_mults[i])
                        except:
                            dists.append(0)
                            dist_mults.append(1)
    ...
    return np.array(xa), np.array(xa_err), get_profile, cont_flags,\
           ...
           np.array(dists), np.array(dist_mults)
    ...
    def get_profile(H_model, P_model, T_model, VMR_model, A_model, PARA_model, gas_id, iso_id, planet_parameters, xn):
        ...
        for i,param in enumerate(param_list):
            ...
            if apr_gas_id[i]<0 and apr_iso_id[i]==0: # Aerosol
                ...
                r_int = np.array([0.015*np.min(haze_wave_grid), 0.0, 0.015*np.min(haze_wave_grid)]) 
