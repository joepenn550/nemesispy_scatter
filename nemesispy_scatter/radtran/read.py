...
def read_tables(filepaths):
    ...
    """
    k_gas_w_g_p_t, gas_id_list, iso_id_list, lta_flags = [], [], [], []
    for filepath in filepaths:
        if filepath.endswith('.kta'):
            gas_id, iso_id, wave_grid, g_ord, del_g, P_grid, T_grid, k_g = read_kta(filepath)
            lta_flags.append(False)
        else:
            gas_id, iso_id, wave_grid, g_ord, del_g, P_grid, T_grid, k_g = read_lta(filepath)
    ...
    return (np.float32(gas_id_list), np.float32(iso_id_list), wave_grid, g_ord,
            del_g, P_grid, T_grid, np.float32(k_gas_w_g_p_t), np.array(lta_flags))
